...
public class ReservationDateExceptionServiceImpl implements ReservationDateExceptionService{
    ...
    public Boolean isValid(ReservationDateException resExcept, BindingResult result){
        if(resExcept.getdescription() == null){
            String msg = messageSource.getMessage("reservationDateException.descriptionNotValid", new Object[]{}, LocaleContextHolder.getLocale());
            ...
            String msg = messageSource.getMessage("reservationDateException.startDateNotValid", new Object[]{}, LocaleContextHolder.getLocale());
            ...
            return false;
        } else if(isEndDateBeforeBeginDate(resExcept, result)){
            return false;
        }
        return true;
    }
        ...
        if(resExcept.getDatesOfReservationDateException().size() > 1){
            for(int i = 0; i < resExcept.getDatesOfReservationDateException().size(); i++){
                ...
                if(getExceptionDates().contains(cal)){
                    String msg = messageSource.getMessage("reservationDateException.oneOrMoredatesExists", new Object[]{}, LocaleContextHolder.getLocale());
            ...
            if(getExceptionDates().contains(cal)){
                String msg = messageSource.getMessage("reservationDateException.startDateExistst", new Object[]{}, LocaleContextHolder.getLocale());
                result.addError(new FieldError(result.getObjectName(), "endDate",
                    null, false,
                    null, null, msg));
                return true;
    ...
    public Boolean isEndDateBeforeBeginDate(ReservationDateException resExcept, BindingResult result){
        if(resExcept.getEndDate() != null) {
            if (resExcept.getEndDate().before(resExcept.getStartDate())) {
                String msg = messageSource.getMessage("reservationDateException.endDateBeforeBeginDate", new Object[]{}, LocaleContextHolder.getLocale());
                result.addError(new FieldError(result.getObjectName(), "endDate",
                    null, false,
                    null, null, msg));
                return true;
